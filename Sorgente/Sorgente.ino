...
void CtrlPiattaformaEnd(bool &stop) {
  if (LetturaNFC != "Nothing") {
    bt_HC05.SendString(LetturaNFC);
    delay(300);
    Lettura_bt = bt.ReadString();
    if (Lettura_bt == "end") {
      stop = true;
    }
  }
...
void loop() {
  ...
  while (LightVar_Sx == 0 && LightVar_Ce == 1 && LightVar_Dx == 0) { // case 010
    Motori.Avanti();
    delay(50);
    LettureLuce();
    Distanza_HCSR04();
  ...
  while (LightVar_Sx == 1 && LightVar_Ce == 1 && LightVar_Dx == 1) { // case 111
    Motori.Avanti();
    delay(50);
    LettureLuce();
    Distanza_HCSR04();
